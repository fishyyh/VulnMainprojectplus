...
func (s *TeamService) GetTeamList(userID uint, roleCode string, page, pageSize int, keyword string) (*TeamListResponse, error) {
	...
	for i := range teams {
		...
		db.Model(&models.Vulnerability{}).
			Where("deleted_at IS NULL AND team_id = ? AND status NOT IN (?)", teams[i].ID, []string{"closed", "completed"}).
			Count(&vulnCount)
...
func (s *TeamService) GetTeamByID(teamID, userID uint, roleCode string) (*models.Team, error) {
	...
	var vulnCount int64
	db.Model(&models.Vulnerability{}).
		Where("deleted_at IS NULL AND team_id = ? AND status NOT IN (?)", team.ID, []string{"closed", "completed"}).
		Count(&vulnCount)
