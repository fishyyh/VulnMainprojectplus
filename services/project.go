...
func (s *ProjectService) GetProjectList(req *ProjectListRequest) (*ProjectListResponse, error) {
	...
	// 如果不是超级管理员，需要过滤项目
	if req.RoleCode != "super_admin" && req.RoleCode != "admin" && req.RoleCode != "security_engineer" {
...
func (s *ProjectService) DeleteProject(projectID, userID uint, roleCode string) error {
	...
	var vulnCount int64
	db.Model(&models.Vulnerability{}).Where("deleted_at IS NULL AND project_id = ? AND status NOT IN (?)", projectID, []string{"completed", "ignored"}).Count(&vulnCount)
	if vulnCount > 0 {
...
func (s *ProjectService) UpdateProjectStats(projectID uint) error {
	...
	db.Model(&models.Vulnerability{}).Where("deleted_at IS NULL AND project_id = ?", projectID).Count(&totalVulns)
	db.Model(&models.Vulnerability{}).Where("deleted_at IS NULL AND project_id = ? AND status = ?", projectID, "unfixed").Count(&unfixedVulns)
	db.Model(&models.Vulnerability{}).Where("deleted_at IS NULL AND project_id = ? AND status = ?", projectID, "fixing").Count(&fixingVulns)
	db.Model(&models.Vulnerability{}).Where("deleted_at IS NULL AND project_id = ? AND status = ?", projectID, "fixed").Count(&fixedVulns)
	db.Model(&models.Vulnerability{}).Where("deleted_at IS NULL AND project_id = ? AND status = ?", projectID, "retesting").Count(&retestingVulns)
	db.Model(&models.Vulnerability{}).Where("deleted_at IS NULL AND project_id = ? AND status = ?", projectID, "completed").Count(&completedVulns)
	db.Model(&models.Vulnerability{}).Where("deleted_at IS NULL AND project_id = ? AND status = ?", projectID, "ignored").Count(&ignoredVulns)
	...
	db.Model(&models.Vulnerability{}).Where("deleted_at IS NULL AND project_id = ? AND severity = ?", projectID, "critical").Count(&criticalVulns)
	db.Model(&models.Vulnerability{}).Where("deleted_at IS NULL AND project_id = ? AND severity = ?", projectID, "high").Count(&highVulns)
	db.Model(&models.Vulnerability{}).Where("deleted_at IS NULL AND project_id = ? AND severity = ?", projectID, "medium").Count(&mediumVulns)
	db.Model(&models.Vulnerability{}).Where("deleted_at IS NULL AND project_id = ? AND severity = ?", projectID, "low").Count(&lowVulns)
	db.Model(&models.Vulnerability{}).Where("deleted_at IS NULL AND project_id = ? AND severity = ?", projectID, "info").Count(&infoVulns)
