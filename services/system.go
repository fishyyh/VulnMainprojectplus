...
func (s *SystemService) GetSystemStats() (map[string]interface{}, error) {
	...
	db.Model(&models.Vulnerability{}).Where("deleted_at IS NULL").Count(&totalVulns)
	db.Model(&models.Vulnerability{}).Where("deleted_at IS NULL AND severity = ?", "critical").Count(&criticalVulns)
	db.Model(&models.Vulnerability{}).Where("deleted_at IS NULL AND severity = ?", "high").Count(&highVulns)
	db.Model(&models.Vulnerability{}).Where("deleted_at IS NULL AND status = ?", "pending").Count(&pendingVulns)
